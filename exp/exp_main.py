...
class Exp_Main(Exp_Basic):
    ...
    def vali(self, train_data, vali_loader, criterion, epoch):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_cycle) in enumerate(vali_loader):
                ...
                if train_data.scale:
                    shape = outputs.shape
                    outputs = np.array([train_data.inverse_transform(output) for output in outputs]).reshape(shape)
                    batch_y = np.array([train_data.inverse_transform(by) for by in batch_y]).reshape(shape)
                pred = outputs
                true = batch_y
                ...
                if i % 1 == 0:
                    ...
                    if train_data.scale:
                        shape = input.shape
                        input = np.array([train_data.inverse_transform(x) for x in input]).reshape(shape)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            vali_loss = self.vali(train_data, vali_loader, criterion, epoch)
